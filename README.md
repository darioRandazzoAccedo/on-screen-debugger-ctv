...
A Husky pre-commit hook runs ESLint + Prettier on staged TypeScript files and Stylelint + Prettier on staged SCSS files.

### Build details

The build uses [tsup](https://tsup.egoist.dev/) with a custom Sass → PostCSS modules plugin that compiles `.scss` files into CSS Modules and injects styles at runtime. Type declarations are emitted separately via `tsc --project tsconfig.build.json`.
