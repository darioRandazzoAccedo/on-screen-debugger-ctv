...
export const toValidOrientation = (
  ...
  const orientationKeys = Object.keys(Orientations);

  for (let i = 0; i < orientationKeys.length; i += 1) {
    const orientationKey = orientationKeys[i] as keyof typeof Orientations;
...
export const findChildWithClosestIndex = (
  ...
  if (
    indexRange &&
    node.activeChild &&
    node.activeChild.index! >= indexRange[0] &&
    node.activeChild.index! <= indexRange[1] &&
...
export const insertChildNode = (parent: Node, newChild: Node): void => {
  ...
  if (
    ...
    for (let i = newChild.index + 1; i < parent.children.length; i += 1) {
 ...
 *
 * @param startNode - node that is the root of traversed subtree
 * @param nodeProcessor - callback executed for traversed node, if true is returned then subtree traversal is interrupted
 */
export const traverseNodeSubtree = <NodeType extends Tree<NodeType>>(
  ...
  while (stack.length > 0) {
    ...
    if (traversedNode.children) {
      for (let i = traversedNode.children.length - 1; i >= 0; i -= 1) {
