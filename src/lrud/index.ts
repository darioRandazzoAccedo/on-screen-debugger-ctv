 ...
 * LRUD v8.0.0
 *
 * Modifications made by Accedo:
 *
 * - Update `blur` & `onBlur` events to receive both the node being blured, and
 *   the `newNode` where the focus will go to.
 *   This allows logic that needs to know the new focus id, without having to create
 *   a microtask (`setTimeout`).
 */
...
export class Lrud {
   ...
   */
  off(eventName: string, callback: Handler<Node>): void {
   ...
   *
   * @param {string|object} nodeArg - node or id of the node that is about to change parent
   * @param {string|object} newParentNodeArg - node or id of the node that became a new parent for nodeId
  ...
  moveNode(
    ...
    const newParentNode =
      typeof newParentNodeArg === 'string'
        ? this.getNode(newParentNodeArg)
        : (newParentNodeArg as Node);
  ...
  unregisterNode(
    ...
    traverseNodeSubtree(node, traversedNode => {
      ...
      for (
        let i = 0, overrideSources = traversedNode.overrideSources || [];
    ...
    this.emitter.emit('blur', { node, newNode: undefined });

    if (node.onBlur) {
      node.onBlur({ node, newNode: undefined });
    }
  ...
  unregisterOverride(
    ...
    for (let i = 0; i < directionKeys.length; i += 1) {
      const directionKey = directionKeys[i] as keyof typeof Directions;
      ...
      const target: Node | undefined = source.overrides[directionToRemove];

      if (target && target.overrideSources) {
        for (let o = 0; o < target.overrideSources.length; o += 1) {
          if (
            target.overrideSources[o].direction === directionToRemove &&
            target.overrideSources[o].node === source
          ) {
   ...
   */
  climbUp(node: Node | undefined, direction: Direction): Node | undefined {
    while (node) {
  ...
  digDown(
    ...
    while (node) {
      ...
      if (
        ...
        (node.isStopPropagate || !this.doesNodeHaveFocusableChildren(node))
      ) {
        return node;
      }
      ...
      */
      ...
      if (this.isIndexAlignMode) {
        ...
        const currentFocusedIndex = currentFocusedIndexRange
          ? currentFocusedIndexRange[0]
          : currentFocusedNode?.index;
  ...
  getNextFocusableChild(node: Node): Node | undefined {
    ...
    if (node.isWrapping) {
      for (let i = 0; i < node.activeChild.index!; i += 1) {
        if (this.isNodeFocusableCandidate(node.children[i])) {
          return node.children[i];
        }
      }
  ...
  getPrevFocusableChild(node: Node): Node | undefined {
    ...
    for (let i = node.activeChild.index! - 1; i >= 0; i -= 1) {
      if (this.isNodeFocusableCandidate(node.children[i])) {
        return node.children[i];
      }
    }
   ...
   */
  getNodeLastChild(node: Node | undefined): Node | undefined {
    if (!node || !node.children) {
      return undefined;
    }
  ...
  handleKeyEvent(
    ...
    const direction = event.keyCode
      ? getDirectionForKeyCode(event.keyCode)
      : toValidDirection(event.direction!);
   ...
   *
   * @param {string|object} parentArg - node or id of the node, which activeChild is about to be unset
   * @param {string|object} activeChildArg - node or id of the node, that is about to be unset as parent's activeChild
   */
  unsetActiveChild(
    ...
    while (parent && parent.activeChild) {
      ...
      if (
        isActiveChildAtCurrentFocusNodeBranch ||
        (isParentAtActiveChildBranch && !isParentAtCurrentFocusedNodeBranch)
      ) {
  ...
  assignFocus(nodeArg: NodeId | Node): void {
    ...
    if (this.currentFocusNode) {
      this.emitter.emit('blur', { node: this.currentFocusNode, newNode: node });

      if (this.currentFocusNode.onBlur) {
        this.currentFocusNode.onBlur({
          node: this.currentFocusNode,
          newNode: node,
        });
      }
    }
  ...
  registerTree(subTreeRootNodeConfig: NodeConfig): void {
    ...
    traverseNodeSubtree(subTreeRootNodeConfig, traversedNodeConfig => {
      ...
      if (traversedNodeConfig.children) {
        for (let i = 0; i < traversedNodeConfig.children.length; i += 1) {
   ...
   *
   * @param {string|object} parentArg - parent or id of the examined parent
   ...
   *
   * @param {string|object} nodeArg - node or id of the node, which focusability property is about to be changed
